...
class Cart(object):
    def __init__(self, request):
        ...
        # запоминаем текущую сессию
        self.session = request.session
        # пытаемся получить данные корзины
        cart = self.session.get(settings.CART_SESSION_ID)
        if not cart:
            # Если не получаем объект корзины, создаем ее как пустой словарь в сессии (Сохраняем в сессии пустую корзину).
    ...
    def __iter__(self):
        ...
        cart = self.cart.copy() # cоздаем копию объекта корзины, получаем товары, сохраненные в ней.
